...
class MatplotlibPlotter(object):
    ...
    def plotPowerCurveSensitivityVariationMetrics(self):
        try:
            from matplotlib import pyplot as plt
            plt.ioff()
            vars_to_plot = [c for c in self.analysis.powerCurveSensitivityVariationMetrics.index if c !=
                            'Significance Threshold']
            (self.analysis.powerCurveSensitivityVariationMetrics.loc[vars_to_plot].dropna()*100.).plot(kind='bar',
                title='Summary of Power Curve Variation by Variable. Significance Threshold = %.2f%%' % (
                    self.analysis.sensitivityAnalysisThreshold * 100), figsize=(12, 8), fontsize=8)
